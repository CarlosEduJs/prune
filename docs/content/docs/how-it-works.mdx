...
prune.yaml
    │
    ▼
scan.Collect()          ← walks filesystem, applies include/exclude globs
    │
    ▼
js.Collector.Collect()  ← reads each file, runs Tree-sitter, extracts AST data
    │
    ▼
js.applyRules()         ← correlates definitions and imports, emits findings
    │
    ▼
report.Formatter        ← formats findings as table, JSON, or NDJSON
```
