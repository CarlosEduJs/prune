...
Shows only findings matching the specified confidence level. Unlike `--min-confidence` (which filters out lower levels), `--only` shows **only** that level:
