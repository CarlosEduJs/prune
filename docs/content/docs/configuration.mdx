...
If you're upgrading from config version `1`, the only required change is bumping the version number:
...
Version `2` adds support for path aliases via `ts_config`. Configs using version `1` continue to work — Prune only requires `version >= 1`.
...
Prune returns an error if this field is absent or set to `0`.
